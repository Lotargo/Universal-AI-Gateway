...
class SmartSearchTool:
    ...
    def _clean_json_string(self, text: str) -> str:
        """
        Robustly cleans a string to extract JSON content, handling Markdown code blocks.
        """
        text = text.strip()
        # Simple case: Wrapped in ``` ... ``` or ```json ... ```
        if "```" in text:
            try:
                parts = text.split("```")
                # Look for a part that looks like JSON or is the main block
                # Usually parts[1] is the content if it starts with ```
                for part in parts:
                    ...
                    if "\n" in clean_part:
                        first_line, rest = clean_part.split("\n", 1)
                        if first_line.strip().lower() in ["json", "text"]:
                            clean_part = rest.strip()
        ...
        # If no markdown blocks found or extraction failed, return original (cleaned)
        return text
    ...
    async def _call_model(self, system_prompt: str, user_content: str, role: str = "worker", specific_model: str = None, retry_count: int = 0, json_mode: bool = False) -> str:
        ...
        try:
            ...
            if response.status_code == 429:
                ...
                # If we have a specific model forced (e.g. by worker logic), we might be stuck on it.
                # If rotation is working, retrying same alias should get a new key.
                # Increase backoff to allow quarantine to process.
                await asyncio.sleep(2.0 * (retry_count + 1))

                # Attempt to switch to a fallback model if we are stuck
                # We wait until at least 3 attempts (retry_count 0, 1, 2) have failed on the same provider
                # before switching. This allows key rotation to work for providers with multiple keys.
                if retry_count > 2 and role in SMART_SEARCH_CONFIG:
                    candidates = SMART_SEARCH_CONFIG[role].get("main", []) + SMART_SEARCH_CONFIG[role].get("fallback", [])
                    if len(candidates) > 1:
                         # Pick a different model to force provider/key switch if possible
                         new_model = random.choice(candidates)
                         if new_model != specific_model and new_model != alias_to_try:
                             logger.info(f"SmartSearch: 429 hit. Switching model from {specific_model or alias_to_try} to {new_model}")
                             return await self._call_model(system_prompt, user_content, role, specific_model=new_model, retry_count=retry_count + 1, json_mode=json_mode)

                return await self._call_model(system_prompt, user_content, role, specific_model, retry_count + 1, json_mode)

            if response.status_code != 200:
                # Special handling for 400 Bad Request caused by unsupported JSON Mode
                # Some models (e.g. gemma-3-27b-it) return 400 when json_mode is requested but not supported.
                if response.status_code == 400 and json_mode:
                    error_msg = response.text.lower()
                    if "json mode" in error_msg or "invalid_argument" in error_msg:
                        logger.warning(f"SmartSearch: 400 Error with JSON Mode ({provider}/{real_model_id}). Retrying with JSON Mode DISABLED.")
                        # Release key (was not quarantined yet)
                        await self.key_manager.release_key(provider, api_key)
                        # Retry immediately without json_mode, keeping same retry_count (logic error, not network/rate error)
                        return await self._call_model(system_prompt, user_content, role, specific_model, retry_count, json_mode=False)
    ...
    async def search(self, query: str) -> str:
        ...
        planner_sys = (
            ...
            f"Break down the user query into 2 to {DEFAULT_NUM_QUERIES} distinct, specific Google search queries. "
            "Return ONLY raw JSON. Do not use Markdown blocks.\n"
            "Example Format: {\"queries\": [\"query 1\", \"query 2\"]}"
        )
        planner_resp = await self._call_model(planner_sys, query, role="planner", json_mode=True)

        try:
            clean = self._clean_json_string(planner_resp)
            ...
            # Heuristic Repair: Try to find list of strings
            try:
                # Matches ["query 1", "query 2"] pattern even if json is broken
                found_lists = re.findall(r'\[(.*?)\]', planner_resp, re.DOTALL)
                if found_lists:
                    # Take the longest list found (likely the queries)
                    candidate = max(found_lists, key=len)
                    # Extract quoted strings
                    extracted_queries = re.findall(r'"([^"]+)"', candidate)
                    if extracted_queries:
                         logger.info(f"SmartSearch: Recovered {len(extracted_queries)} queries via regex heuristic.")
                         search_queries = extracted_queries
                    else:
                        search_queries = [query]
                else:
                    search_queries = [query]
        ...
        async def worker_task(sub_query):
            async with semaphore:
                ...
                worker_sys = (
                    ...
                    "Example: {\"summary\": \"The movie...\", \"facts\": [\"Fact 1\", \"Fact 2\"], \"sources\": [{\"title\": \"Site\", \"url\": \"http...\"}]}"
                )
                ...
                # Clean response (Worker might return markdown if JSON mode failed)
                return self._clean_json_string(worker_resp)
        ...
        for res in worker_results_json:
            try:
                # Result is already cleaned string, try to load
        ...
        aggregator_sys = (
            ...
            "Return ONLY raw JSON.\n"
            "Example: {\"final_answer\": \"Based on reviews...\", \"meta_comment\": \"Data from 2025 confirmed.\"}"
        )
        final_resp = await self._call_model(aggregator_sys, f"User Request: {query}\n\nWorker Reports:\n{context_str}", role="aggregator", json_mode=True)

        try:
            clean_final = self._clean_json_string(final_resp)
            final_obj = json.loads(clean_final)
